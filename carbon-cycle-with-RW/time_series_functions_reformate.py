...
from Si_cycle_functions import RW_flux,Si_Bio_flux,Si_aBio_flux
...
# The input parameters to Forward_Model are defined in main_code_parallel.py.
def Forward_Model(W,F_outgass,n,climp,tdep_weath,mod_sea,alt_frac,Mp_frac,lfrac,carb_exp,sed_thick,F_carbw,fpel,CWF,deep_grad,coef_for_diss,beta,n_out,mm,growth_timing,new_add_Ca,Ebas,AlkSi_ratio,initRW,Si_init):

    # define global variables:
    global pK1,pK2,H_CO2,salt,Pore_mod,T_surface0,Ca_p,Ca_o,options_array,ALK_to_Si
    global landf_vary,lum_vary,salt0,k_w,k_c,k_o,k_r,Mp,ppCO2_o,Mo,ko1,ko2,ALK_o,ALK_p,Si_ALK_weather,pH_o,pH_transition
    ...
    F_sil0=(1-partition)*F_outgass+(1-alt_frac)*partition*F_outgass #+ 0.5*ALK_to_Si*0.545 #initial continental silicate weathering flux
    F_sil0 = (1-partition)*F_outgass + 0.5*ALK_to_Si * initRW  ## new RW initial con
    Fprecip_p=partition*F_outgass #initial pore space carbonate precipation flux (reduntant)
    
    # initialize Si cycle
    Si_ALK_weather = 0.5 #In weathering reactions, Si released per mol ALK 0.5 ->two alk for every one silicon in continental weathering?  
    Si_guess = Si_init
    
    Si_0 = Si_guess 

    # Iteratively adjust outgassing and silica weathering to find initial conditions  
    Si_0 = 0.05/1000.0 #impose approximate modern ocean concentrations
    initialRW = RW_flux(Si_0,10**-pH_o,0.0,initRW)
    F_diss0L = F_diss0
    partitionL = partition
    Fprecip_pL = Fprecip_p
    for jj in range(0,20):
        F_sil0L = (initialRW + Si_Bio_flux(Si_0,0) + Si_aBio_flux(Si_0,0,T))/(Si_ALK_weather*2) - F_diss0L
        F_outgassL = (F_sil0L - 0.5*ALK_to_Si * initialRW)/(1-partitionL)
        partitionL=mod_sea/F_outgassL
        F_diss0L=alt_frac*partitionL*F_outgassL #initial seafloor dissolution flux i.e. dissolution = alt_frac*precip_seafloor
        Fprecip_pL=partitionL*F_outgassL

    F_sil0 = F_sil0L
    F_diss0 = F_diss0L
    F_outgass = F_outgassL
    Fprecip_p = Fprecip_pL
    ...
    Ca_o=10*0.0100278 # initial (modern) calcium molality
    ...
    [out,mes]=scipy.integrate.odeint(system_of_equations, [DIC_o+1.8e20/Mo*ppCO2_o,ALK_o,DIC_p,ALK_p,Si_0], time, args=(W,F_outgass,n,climp,tdep_weath,mod_sea,alt_frac,Mp_frac,lfrac,carb_exp,sed_thick,F_carbw,CWF,deep_grad,coef_for_diss,beta,n_out,mm,growth_timing,new_add_Ca,Ebas,initRW,Si_init),full_output=1)
    ...
    F_reverse_ar = 0*time # RW flux
    Si_Bio_ar = 0*time # Biogenic silica sink
    Si_abiotic_ar = 0*time # Abiotic silica sink
    ...
    for i in range(0,len(time)): 
        ...
        pl1=carbon_cycle([out[i,0],out[i,1],out[i,2],out[i,3],out[i,4]],time[i],W,F_outgass,n,climp,tdep_weath,mod_sea,alt_frac,Mp_frac,lfrac,carb_exp,sed_thick,F_carbw,CWF,deep_grad,coef_for_diss,beta,n_out,mm,growth_timing,new_add_Ca,Ebas,initRW,Si_init) #ocean
        ...
        F_reverse_ar[i] = pl1[28]
        Si_Bio_ar[i] = pl1[29]
        Si_abiotic_ar[i] = pl1[30]
    ...
    return [out[:,0],out[:,1],out[:,2],out[:,3],time,pH_array_o,CO2_array_o,pH_array_p,CO2_array_p,Ca_array_o,Ca_array_p,CO3_array_o,CO3_array_p,HCO3_array_o,HCO3_array_p,omega_o,omega_p,Tsurf_array,Tdeep_array,Fd_array,Fs_array,Precip_ocean_array,Precip_pore_array,spread_ar,volc_array,T_pore_space_ar,F_reverse_ar,Si_Bio_ar,Si_abiotic_ar,out[:,4]],imbalance
...
def system_of_equations(y,t0,W,F_outgass,n,climp,tdep_weath,mod_sea,alt_frac,Mp_frac,lfrac,carb_exp,sed_thick,F_carbw,CWF,deep_grad,coef_for_diss,beta,n_out,mm,growth_timing,new_add_Ca,Ebas,initRW,Si_init):
   
    global pK1,pK2,H_CO2,salt,Pore_mod,T_surface0,ALK_to_Si,Si_ALK_weather
    global landf_vary,lum_vary,salt0,k_w,k_c,k_o,k_r,Mp,ppCO2_o,Mo,ko1,ko2,pH_o,pH_transition
    ...
    [EE_pH_o,EE_co3_o,EE_hco3_o, EE_co2aq_o,EE_ppCO2_o,EE_Ca_o,EE_omega_o,T_surface,buffer_T,F_diss,F_silic,Precip_ocean,Precip_pore,EE_omega_p,F_outg,carb_weath,EE_co3_p,EE_hco3_p, EE_co2aq_p,EE_pH_p,EE_ppCO2_p,EE_Ca_p,EE_H_o,EE_H_p,T_surface_diff,F_outg,spread_value,T_pore_space,Frw_flux,Si_Bio,Si_abiotic]=carbon_cycle([y[0],y[1],y[2],y[3],y[4]],t0,W,F_outgass,n,climp,tdep_weath,mod_sea,alt_frac,Mp_frac,lfrac,carb_exp,sed_thick,F_carbw,CWF,deep_grad,coef_for_diss,beta,n_out,mm,growth_timing,new_add_Ca,Ebas,initRW,Si_init)
    ...
    dy1_dt=2*carb_weath/Mo-W*(y[1]-y[3])/Mo+2*F_silic/Mo-2*Precip_ocean/Mo -ALK_to_Si*Frw_flux/Mo  # Time derivative of ocean alkalinity
    ...
    dy4_dt=Si_ALK_weather*2*(F_silic + F_diss)/Mo - (Frw_flux + Si_Bio + Si_abiotic)/Mo  ## time derivative of ocean silica concentration
    return [dy0_dt,dy1_dt,dy2_dt,dy3_dt,dy4_dt] # return derivatives           
...
def carbon_cycle (y,t0,W,F_outgass,n,climp,tdep_weath,mod_sea,alt_frac,Mp_frac,lfrac,carb_exp,sed_thick,F_carbw,CWF,deep_grad,coef_for_diss,beta,n_out,mm,growth_timing,new_add_Ca,Ebas,initRW,Si_init):
    ...
    global pK1,pK2,H_CO2,salt,Pore_mod,T_surface0,ALK_o,ALK_p,Ca_p,Ca_o,options_array,pH_o,pH_transition
    ...
    for jk in range(0,max_temp_iter):
        ...
        sed_depth=700-700*(1-sed_thick)*t0/4e9 # equation S5 in Krissansen-Totton et al. (2018; PNAS)
    ...
    # Calculate saturation state of ocean and pore space
    ...
    F_diss=k_r*2.88*10**-14*10**(-coef_for_diss*EE_pH_p)*spread**beta*numpy.exp(-Ebas/(8.314*(T_pore_space)))

    ## Testing pH-sensitivity (Appendix A)
    #if t0<1.041e9:
    #    pH_transition = EE_pH_o  
    #if t0> 1.041e9:
    #    Frw = RW_flux(y[4],EE_H_o,t0,initRW)*land*(EE_pH_o/pH_transition)**22.4 ##### calls external polynomial fit
    #else:
    #    Frw = RW_flux(y[4],EE_H_o,t0,initRW)*land ##### calls external polynomial fit

    ### Nominal case, no pH-dependence reverse weathering fluxes
    Frw = RW_flux(y[4],EE_H_o,t0,initRW)*land #calls polynomial fit to sediment diagenesis model
    #Frw = RW_flux(y[4],EE_H_o,t0,initRW) #sensitivity test, no supply limit on RW
    Si_Bio= Si_Bio_flux(y[4],t0) ##### calls polynomial fit to sediment diagenesis model
    Si_abiotic = Si_aBio_flux(y[4],t0,T_surface) ##### abiotic silica sink parameterization
    F_silic=k_w*biology_modifier*land*(EE_ppCO2_o/ppCO2_o)**climp*numpy.exp((T_surface_diff)/tdep_weath) #continental weathering flux     

    #return selected carbon cycle variables and fluxes   
    return [EE_pH_o,EE_co3_o,EE_hco3_o, EE_co2aq_o,EE_ppCO2_o,EE_Ca_o,EE_omega_o,T_surface,buffer_T,F_diss,F_silic,Precip_ocean,Precip_pore,EE_omega_p,F_outg,carb_weath,EE_co3_p,EE_hco3_p, EE_co2aq_p,EE_pH_p,EE_ppCO2_p,EE_Ca_p,EE_H_o,EE_H_p,T_surface_diff,F_outg,spread,T_pore_space,Frw,Si_Bio,Si_abiotic] #T_pore_space
